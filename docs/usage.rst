...
The following files are generated by PhysioFit in the output directory:
    ...
    * **test_results.tsv**: Results from the χ² statistical test.
...
.. warning:: When the data file is directly used as input in the Graphical User Interface, Physiofit cannot get the path
             directly from the file metadata. For this reason in the generated *config_file.json* the *path_to_data*
             will be set to None.

It is important to check the quality of the fit before interpreting the estimated fluxes. To evaluate the fit quality, you can check the *test_results.tsv*
