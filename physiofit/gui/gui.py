...
class App:
    ...
    def _initialize_opt_menu_widgets(self, input_values):
        ...
        with form:
            st.subheader("Choose run parameters:")
            self.vini = st.number_input(
                "Initial value",
            ...
            st.subheader("Choose outputs to generate:")
            col1, col2, col3 = st.columns(3)
            with col1:
                self.data_out = st.checkbox("Data",
                                       key="data_out",
                                       help="Select this option to export a csv file containing the run report")
            with col2:
                self.plots_out = st.checkbox("Plots",
                                        key="plots_out",
                                        help="Select this option to export plots")
            with col3:
                self.pdf_out = st.checkbox("Pdf",
                                      key="pdf_out",
                                      help="Select this option to export plots in a pdf file")
